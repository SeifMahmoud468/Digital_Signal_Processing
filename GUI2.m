...
if get(handles.IIR,'value') == 1 %IIR
    ...
    %Filter no. 2 range 170hz to 310khz 
    [num2, den2] = butter(3, [2*170/sampleRate,2*310/sampleRate],'bandpass');
    filters = [filters filter(num2,den2,y)];
    %Filter no. 3 range 310khz to 600hz 
    [num3, den3] = butter(3, [2*310/sampleRate,2*600/sampleRate],'bandpass');
    filters = [filters filter(num3,den3,y)];
    %Filter no. 4 range 600hz to 1khz 
    [num4, den4] = butter(3, [2*600/sampleRate,2*1000/sampleRate],'bandpass');
    filters = [filters filter(num4,den4,y)];
    %Filter no. 5 range 1khz to 3khz 
    [num5, den5] = butter(3, [2*1000/sampleRate,2*3000/sampleRate],'bandpass');
    ...
    %Filter no. 9 range 14khz to 16khz 
...
if get(handles.FIR,'value') == 1 %FIR type
    ...
    filters = [filters filter(num1,den1,y)];
    %Filter no. 2 range 170hz to 310khz 
    [num2, den2] = fir1(50, [2*170/sampleRate,2*310/sampleRate],'bandpass');
    filters = [filters filter(num2,den2,y)];
    %Filter no. 3 range 310khz to 600hz 
    [num3, den3] = fir1(50, [2*310/sampleRate,2*600/sampleRate],'bandpass');
    filters = [filters filter(num3,den3,y)];
    %Filter no. 4 range 600hz to 1khz 
    [num4, den4] = fir1(50, [2*600/sampleRate,2*1000/sampleRate],'bandpass');
    filters = [filters filter(num4,den4,y)];
    %Filter no. 5 range 1khz to 3khz 
    [num5, den5] = fir1(50, [2*1000/sampleRate,2*3000/sampleRate],'bandpass');
    filters = [filters filter(num5,den5,y)];
    %Filter no. 6 range 3khz to 6khz 
    ...
    filters = [filters filter(num8,den8,y)];
    %Filter no. 9 range 14khz to 16khz 
    [num9, den9] = fir1(50, [2*14000/sampleRate,2*16000/sampleRate],'bandpass');
    filters = [filters filter(num9,den9,y)];
end
...
if choice == 1
    while(1)
        choice1=menu('Choose a filter', '0 - 170 Hz','170 - 310 Hz','310 - 600 Hz','600 - 1000 Hz','1 - 3 KHz','3 - 6 KHz','6 - 12 KHz','12 - 14 KHz','14 - 16 KHz','Exit');
        if choice1 ~=10
            switch choice1
                case 1
                    a = num1;
